---
name: git-workflow
description: 'Enforce git workflow rules and guardrails — single-trunk branching, commits, pushes, PRs, merge discipline, submodules, and release flow. Use whenever the user or agent touches git: creating branches/worktrees, committing, pushing, merging, tagging releases, handling submodules, or asks about git rules, commit conventions, or "boleh commit/push?". Dotfiles override: control-checkout work commits directly on `main` (no branch/PR); worktree-isolated work keeps branch → push → PR → merge.'
---

# Git Workflow — Rules & Guardrails (dotfiles override)

Project override for the dotfiles repo. The global `git-workflow` skill keeps
its strict rule (`main` never receives direct commits); **in this repo**, work
done in the control checkout (the main worktree) commits directly to `main`.
Worktree-isolated work (`.worktrees/`, `/goal` lanes, parallel/risky changes)
keeps the strict flow: branch → push → PR → merge. Everything below mirrors
`config/skills/git-workflow/SKILL.md` with those amendments — keep the two in
sync when the global skill changes.
...
- `main` is single trunk. **Control checkout:** work and commit directly on `main` — no branch, no PR. **Worktree work** (parallel/risky/lanes): start from `main` in a separate branch, then PR → merge. Never commit to `main` from inside a worktree.
- Branch naming (worktree work):
...
- Flow — control checkout: `git pull --ff-only` when trunk moved, then work and commit on `main`. Worktree: `git worktree add -b <branch> .worktrees/<slug> main`; keep the branch rebased on `main` (`git fetch && git rebase origin/main`) — never rewrite `main`.
...
## 7. Release (project-declared)

Follow the project's release doc — never improvise:

1. Fill the project's changelog(s) with dated sections.
2. Run the project's full gate.
3. **Control checkout:** commit `chore(release): vX.Y.Z` (one commit, all bumps) directly on `main`, then tag after the gate is green and the user approves the tag push:
   ```bash
   git tag -a vX.Y.Z -m "<one-line summary>"
   git push origin vX.Y.Z   # only after user approval
   ```
   **Worktree:** branch `chore/release-vX.Y.Z` from `main` → commit on branch → push → PR to `main` → merge after review, then tag on `main` (after pull).
   Multiple release artifacts (e.g. web + CLI) use independent tags, bumped in the same release commit.

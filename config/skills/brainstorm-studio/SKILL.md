...
## Graph-first design (design-thinking)

The design you present is the graph, not prose about it. Follow the `design-thinking` paradigm: read the problem, draw the graph, build what IS the graph. At its core is A/E/R — the happy-path graph (A), the break points (E), and the requirements/dependencies (R). That is not a backend concern: any problem whose logic must be right — a service, an API, an algorithm, a state machine, a pipeline, a data flow, error handling, tests, a sequence of steps — is drawn the same way and built as the graph. Draw each section before presenting it; if the eventual code does not match the drawn graph, the code is wrong — fix the code or fix the graph, never leave them disagreeing.
...
`skills/brainstorm-studio/visual-companion.md`

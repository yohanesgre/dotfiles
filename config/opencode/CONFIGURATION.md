...
| researcher | 40 | read/glob/grep/list/skill + webfetch/websearch allow; subagent deny; model `opencode-go/mimo-v2.5`. Routes: `explorer` (codebase) / `librarian` (external) |
...
| designer | 50 | read + edit + skill allow; shell ask; web ask; question allow; subagent deny. Design only — no implementation source; design-artifact paths + build gate come from the project's `AGENTS.md`/`.opencode` |

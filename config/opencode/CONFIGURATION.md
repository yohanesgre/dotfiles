...
> 2026-09-12 — `brainstorm-studio`/`brainstorming`: the writing-plans handoff is now optional — the approved spec is a valid terminal state. Flow gains a "User wants a plan?" decision before invoking writing-plans; checklist/implementation sections ask first instead of mandating it. Spec is no longer committed or pushed by the skill: it stays unstaged for the user to review and commit (`commit the design document` removed; review-gate copy now says "not committed"). `architect` routes to writing-plans only when a plan is requested. Files: `config/skills/{brainstorm-studio,brainstorming,architect}`.
